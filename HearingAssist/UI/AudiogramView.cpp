...
{
    // 1. Setup Audiogram Graph Visualizer
    addAndMakeVisible(audiogramGraph);

    // 2. Setup 12 Vertical Sliders (6 Left in Blue, 6 Right in Red)
    ...
    // 4. Prescription Formula Dropdown
    ...
    updateModeButtons(2); // Default to Amplification ON

    // 6. Master & Tone Sliders
    ...
    // Initial preset load
    selectPreset(3); // Moderate Presbycusis

    setSize(780, 480);
    startTimerHz(25); // 25 FPS Metering UI update
}
...
void AudiogramView::paint(juce::Graphics& g)
{
    ...
    g.drawText("HEARING ASSIST - PERSONALIZED SYSTEM AUDIO AMPLIFIER", 20, 8, getWidth() - 40, 42, juce::Justification::left);

    // Section Outlines
    g.setColour(juce::Colour(0xFF2D3748));
    g.drawRoundedRectangle(12.0f, 56.0f, 370.0f, 210.0f, 6.0f, 1.0f); // Audiogram plot box
    g.drawRoundedRectangle(390.0f, 56.0f, static_cast<float>(getWidth() - 402), 210.0f, 6.0f, 1.0f); // Sliders box
    g.drawRoundedRectangle(12.0f, 274.0f, static_cast<float>(getWidth() - 24), 160.0f, 6.0f, 1.0f); // Controls box
...
{
    // Left Top: Audiogram Graph Plot
    audiogramGraph.setBounds(18, 62, 358, 198);

    // Right Top: 12 Sliders
    int sliderY = 82;
    int sliderH = 150;
    int spacing = 58;
    int startX = 405;
    ...
    // Bottom Area: Toolbar & Fine Controls
