...
int AudiogramComponent::dbToScreenY(float dbHL) const {
    // In clinical audiograms, 0 dB is at top, 100 dB is at bottom
...
void AudiogramComponent::paint(juce::Graphics& g) {
    ...
    // Normal Hearing Region (-5 to 20 dB HL)
    ...
    // Draw Left Ear Curve (Blue Line + Crosses)
    juce::Path leftPath;
    for (size_t i = 0; i < 6; ++i) {
        int x = freqToScreenX(i);
    ...
    // Draw Right Ear Curve (Red Line + Circles)
    juce::Path rightPath;
    for (size_t i = 0; i < 6; ++i) {
        int x = freqToScreenX(i);
